...
> 目标：收盘后补齐最新选股批次的 K 线，并计算 Top N 预览；默认不写入周末 Top/复盘表。
...
3. 下载/更新该批次股票的日 K。
4. 使用最新批次执行规则打分，输出 Top N 预览。
5. 默认不新增 `weekly_screen_runs`、`weekly_selected_stocks`、ML 预测和复盘结果。
...
# 恢复旧行为：日更时也写入 Top N、训练 ML，并允许新批次切换时自动复盘上一期
...
| 文件/目录 | 说明 |
|-----------|------|
| `run_daily_update.sh` | **工作日一键流程**：补 K 线 + Top N 预览，默认不落库；可用环境变量恢复持久化 |
